...
# This script installs puppet 4.x and deploy the manifest using puppet apply -e "include profile_puppetmaster"
# Usage:
# <powershell>
# Set-ExecutionPolicy Unrestricted -Force
# icm $executioncontext.InvokeCommand.NewScriptBlock((New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/relybv/dirict-profile_puppetmaster/master/files/bootme.ps1')) -ArgumentList ("profile_puppetmaster")
#</powershell>


  param(
    [string]$role = "profile_puppetmaster"
  )

  $puppet_source = "https://github.com/relybv/dirict-profile_puppetmaster.git"
  ...
  if(!(Test-Path -Path "C:\ProgramData\PuppetLabs\code\modules\profile_puppetmaster" )){
    $clone_args = @("clone",$puppet_source,"C:\ProgramData\PuppetLabs\code\modules\profile_puppetmaster" )
  } else {
    $clone_args = @("pull")
    Set-Location -Path "C:\ProgramData\PuppetLabs\code\modules\profile_puppetmaster"
  }
